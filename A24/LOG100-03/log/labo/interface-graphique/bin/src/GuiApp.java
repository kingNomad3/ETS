...
public class GuiApp extends JFrame {
  ...
  private JButton changeGateButton, cancelButton, notifyBoardingButton, removeFlightButton, addButton, delayButton, quitButton;
  // TODO: Add attributes for other operations
  ...
  public void initListeners() {
    ...
    changeGateButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        // Logic to open Change Gate dialog or perform the operation
      }
    });
    ...
    cancelButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        // Logic to open Cancel Flight dialog or perform the operation
      }
    });
    ...
    removeFlightButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        // Logic to open Remove Flight dialog or perform the operation
      }
    });
  ...
  public void addFlightToGate(Flight flight, String terminal, int gateNumber) {
    ...
    if (gates != null && gateNumber > 0 && gateNumber <= gates.length) {
      gates[gateNumber - 1].addFlight(flight);
    } else {
      throw new IllegalArgumentException("Invalid gate number: " + gateNumber);
    }
  }
  ...
  public boolean delayFlight(String company, int flightNumber) {
    // Iterate through all flights in the airport
    for (Flight flight : airport.getFlights()) {
      // Check if the flight matches the company and flight number
      if (flight.getCompany().equalsIgnoreCase(company) && flight.getFlightNumber() == flightNumber) {
        // Update the flight status to "DELAYED"
        flight.setStatus(Flight.DELAYED);

        // Notify observers about the change
        airport.notifyObservers();

        // Return true indicating the flight was delayed successfully
        return true;
      }
    }

    // Return false if no matching flight was found
    return false;
  }

  public boolean changeGate(String company, int flightNumber, String terminal, int gateNumber) {
    // Determine the gate array based on the terminal
    Gate[] gates = null;
    ...
    // Check if the gate number is valid
    if (gateNumber <= 0 || gateNumber > gates.length) {
      throw new IllegalArgumentException("Invalid gate number: " + gateNumber);
    }

    // Find the flight in the airport's flight list
    for (Flight flight : airport.getFlights()) {
      if (flight.getCompany().equalsIgnoreCase(company) && flight.getFlightNumber() == flightNumber) {
        // Remove the flight from the old gate
        String oldGateName = flight.getGate();
        String[] oldGateParts = oldGateName.split("-");
        if (oldGateParts.length == 2) {
          String oldTerminal = oldGateParts[0];
          int oldGateNumber = Integer.parseInt(oldGateParts[1]);
          Gate[] oldGates = null;

          switch (oldTerminal.toUpperCase()) {
            case "A":
              oldGates = gatesA;
              break;
            case "B":
          ...
          if (oldGates != null && oldGateNumber > 0 && oldGateNumber <= oldGates.length) {
            oldGates[oldGateNumber - 1].removeFlight(flight);
          }
        }
  ...
  public boolean cancelFlight(String company, int flightNumber) {
    // Iterate through all flights in the airport
    for (Flight flight : airport.getFlights()) {
      // Check if the flight matches the company and flight number
      if (flight.getCompany().equalsIgnoreCase(company) && flight.getFlightNumber() == flightNumber) {
        ...
        // Remove the flight from its gate
        String gateName = flight.getGate();
        String[] gateParts = gateName.split("-");
        if (gateParts.length == 2) {
          String terminal = gateParts[0];
          int gateNumber = Integer.parseInt(gateParts[1]);
          Gate[] gates = null;
          ...
          if (gates != null && gateNumber > 0 && gateNumber <= gates.length) {
            gates[gateNumber - 1].removeFlight(flight);
          }
        }

        // Remove the flight from the terminal
        String terminalName = gateParts[0];
        switch (terminalName.toUpperCase()) {
          case "A":
        ...
        // Return true indicating the flight was cancelled successfully
        return true;
      }
    }
  ...
  public boolean notifyBoarding(String company, int flightNumber) {
    // Iterate through all flights in the airport
    for (Flight flight : airport.getFlights()) {
      // Check if the flight matches the company and flight number
      if (flight.getCompany().equalsIgnoreCase(company) && flight.getFlightNumber() == flightNumber) {
        // Update the flight status to "BOARDING"
        flight.setStatus(Flight.BOARDING);

        // Notify observers
        airport.notifyObservers();

        // Return true indicating the flight status was successfully updated
        return true;
      }
    }
  ...
  public boolean removeFlight(String company, int flightNumber) {
    // Iterate through all flights in the airport
    for (Flight flight : new ArrayList<>(airport.getFlights())) { // Create a copy to avoid concurrent modification
      // Check if the flight matches the company and flight number
      if (flight.getCompany().equalsIgnoreCase(company) && flight.getFlightNumber() == flightNumber) {
        // Remove the flight from its gate
        String gateName = flight.getGate();
        ...
        // Remove the flight from its terminal
        String terminalName = gateParts[0];
        switch (terminalName.toUpperCase()) {
          case "A":
            termA.removeFlight(flight);
        ...
        // Remove the flight from the airport
        airport.removeFlight(flight);

        // Notify observers
        airport.notifyObservers();

        // Return true indicating the flight was removed successfully
        return true;
      }
    }
